...
<details><summary>Workload Tail flags</summary>

+ *component*(`--component`): Set the component from which the tail command should stream the logs, the values that the flag can take depends on the final deployed pods label `app.kubernetes.io/component`, for example, `build`, `run` and `config-writer`
    ```bash
...
+ *namespace*(`-n`/`--namespace`):  Specifies the namespace where the workload was deployed to get logs from
    ```bash
...
+ *since*(`--since`): Sets the time duration to start reading logs from, this can be set in seconds (`s`), minutes(`m`) or hours (`h`) in the format `0h0m0s`, when the duration is `0` it is net neccesary to be written for example for 1 hour, 0 minutes and 1 seconds is `1h1s`. The default value for this flag is 1 second `1s`
    ```bash
...
+ *timestamp*(`-t`/`--timestamp`): Adds the timestamp to the begining of each log message
    ```bash
