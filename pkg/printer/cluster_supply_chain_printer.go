...
func ClusterSupplyChainPrinter(w io.Writer, clustersupplychain *cartov1alpha1.ClusterSupplyChain) error {
	...
	printClusterSupplyChainSelectors := func(clustersupplychain *cartov1alpha1.ClusterSupplyChain, printOpts table.PrintOptions) ([]metav1beta1.TableRow, error) {
		...
		selectorRows, err := printSelectorRow(clustersupplychain.Spec.LegacySelector.Selector, printOpts)
		if err != nil {
			return nil, err
		}
		rows = append(rows, selectorRows...)
		fieldRows, err := printMatchFieldsRow(clustersupplychain.Spec.LegacySelector.SelectorMatchFields, printOpts)
		if err != nil {
			return nil, err
		}
		rows = append(rows, fieldRows...)
		expressionRows, err := printMatchExpressionsRows(clustersupplychain.Spec.LegacySelector.SelectorMatchExpressions, printOpts)
		if err != nil {
			return nil, err
		}
