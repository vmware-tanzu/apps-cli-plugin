...
func (opts *WorkloadOptions) DefineFlags(ctx context.Context, c *cli.Config, cmd *cobra.Command) {
	...
	cmd.Flags().StringVarP(&opts.ServiceAccountName, cli.StripDash(flags.ServiceAccountFlagName), "s", "", "name of service account permitted to create resources submitted by the supply chain (to unset, pass empty string \"\")")
