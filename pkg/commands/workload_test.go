...
import (
	...
	rcs "k8s.io/cli-runtime/pkg/resource"
...
func TestWorkloadOptionsFetchResourceObject(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			cmd := &cobra.Command{}
			ctx := cli.WithCommand(context.Background(), cmd)
			c.Builder = rcs.NewFakeBuilder(
				func(version schema.GroupVersion) (rcs.RESTClient, error) {
					codec := k8sscheme.Codecs.LegacyCodec(scheme.PrioritizedVersionsAllGroups()...)
					UnstructuredClient := &fake.RESTClient{
						NegotiatedSerializer: rcs.UnstructuredPlusDefaultContentConfig().NegotiatedSerializer,
				...
				func() (restmapper.CategoryExpander, error) {
					return rcs.FakeCategoryExpander, nil
				},
			)
